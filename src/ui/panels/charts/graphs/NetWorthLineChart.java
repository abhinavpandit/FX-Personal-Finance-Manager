...
public class NetWorthLineChart extends GridPane
{
    ...
    public NetWorthLineChart(DataModel dataModel,StringProperty status)
    {
        super();
    ...
    private ObservableList<XYChart.Series<String,Double>> generateChart()
    {
        ...
        XYChart.Series<String,Double> netWorthSeries = new XYChart.Series<>();
       ...
       // datePoint = datePoint.plusMonths(1);
        ...
        {
            String label = datePoint.getDayOfMonth()+"/"+datePoint.getMonth().toString().substring(0,3)+"/"+datePoint.getYear();
            XYChart.Data<String, Double> dataPoint = new XYChart.Data<>(label, getNetWorthAtDate(datePoint));
            netWorthSeries.getData().add(dataPoint);
        ...
        seriesList.add(netWorthSeries);
        
        return seriesList;
              
    }
    private double getNetWorthAtDate(LocalDate date)
    {
        double amount = 0.0;
        for(Account account : dataModel.getAccountList())
        {
            ACCOUNT_TYPE at = account.getAccountType();
            if(at == ACCOUNT_TYPE.ASSET || at == ACCOUNT_TYPE.BANK || at == ACCOUNT_TYPE.CASH)
            {
               
                amount = amount + account.getBalanceAtDate(date);
                //System.out.println("considering account : "+account.getAccountName() +" ; amount = "+amount);
